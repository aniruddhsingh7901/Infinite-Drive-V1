...
exports.deleteBook = exports.getBookById = exports.getBooks = exports.updateBook = exports.addBook = void 0;
                ...
                try {
                    ...
                    if (files.ebooks && files.ebooks.length > formatArray.length) {
                        ...
                        for (const bonusFile of bonusFiles) {
                            ...
                            const matchingBonus = processedBonuses.find((b) => b.fileName === bonusFile.originalname);
                            if (matchingBonus) {
...
const updateBook = async (req, res, next) => {
    upload.fields([
        ...
        try {
            ...
            const files = req.files;
            const filePaths = book.filePaths;
            // Ensure formats is an array
            const formatArray = typeof formats === 'string' ? formats.split(',') : [];
            if (files && files['ebooks']) {
                // Only process the first N files as ebooks, where N is the number of formats
                const ebookFiles = files['ebooks'].slice(0, formatArray.length);
                await Promise.all(formatArray.map(async (format, index) => {
            ...
            if (files && files['coverImage']) {
                book.coverImagePaths = await Promise.all(files['coverImage'].map(file => new Promise((resolve, reject) => {
            ...
            if (bonuses) {
                try {
                    ...
                    if (files && files['ebooks'] && files['ebooks'].length > formatArray.length) {
                        ...
                        for (const bonusFile of bonusFiles) {
                            // Find the corresponding bonus item by filename
                            const matchingBonus = parsedBonuses.find((b) => b.fileName === bonusFile.originalname);
                            const matchingBonusCover = parsedBonuses.find((b) => b.coverImageFileName === bonusFile.originalname);
                            if (matchingBonus) {
...
const deleteBook = async (req, res, next) => {
    try {
        ...
        const book = await Book_1.default.findOne({ where: { id: bookId } });
