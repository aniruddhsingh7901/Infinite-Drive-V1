...
class EmailService {
    constructor() {
        console.log('Email service initialized with SendGrid only');
    }
    /**
    ...
    async sendEmail(to, subject, body, attachments) {
        try {
            ...
            return await sendgridEmailService_1.default.sendEmail(to, subject, body, attachments);
        }
        catch (error) {
     ...
     * @returns Promise resolving to true if email was sent successfully
     */
    async sendDownloadLink(to, downloadDetails) {
    ...
    `;
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f0fff4; padding: 20px; border-radius: 5px; margin: 20px 0;">
            ...
            <ul>
              ${downloadDetails.bonusItems.map((item) => `
     ...
     * @returns Promise resolving to true if email was sent successfully
     */
    async sendContactFormConfirmation(to, contactDetails) {
